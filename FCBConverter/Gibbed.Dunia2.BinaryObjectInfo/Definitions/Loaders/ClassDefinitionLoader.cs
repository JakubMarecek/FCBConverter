...
namespace Gibbed.Dunia2.BinaryObjectInfo.Definitions.Loaders
{
    internal static class ClassDefinitionLoader
    {
        ...
        private static IList<FieldDefinition> LoadFields(IEnumerable<Raw.FieldDefinition> raws)
        {
            ...
            if (raws != null)
            {
                foreach (var raw in raws)
                {
                    ...
                    {
                        throw new LoadException(string.Format("duplicate field '{0}'",
                                                              def.Name));
                    }
                    defs.Add(def);
                }
        ...
        private static EnumDefinition LoadEnum(Raw.EnumDefinition raw)
        {
            ...
            return new EnumDefinition()
            {
                Name = raw.Name,
                Elements =
                    new ReadOnlyCollection<EnumElementDefinition>(LoadEnumElements(raw.Elements)),
            };
        }
        ...
        private static IList<EnumElementDefinition> LoadEnumElements(IEnumerable<Raw.EnumElementDefinition> raws)
        {
            ...
            if (raws != null)
            {
                foreach (var raw in raws)
                {
                    ...
                    {
                        throw new LoadException(string.Format("duplicate element name '{0}'",
                                                              def.Name));
                    }

                    if (defs.Any(fd => fd.Value == def.Value))
                    {
                        throw new LoadException(string.Format("duplicate element value '{0}'",
                                                              def.Value));
                    }
